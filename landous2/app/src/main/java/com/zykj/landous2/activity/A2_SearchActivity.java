 ...
 * 
 */
public class A2_SearchActivity extends Activity implements IXListViewListener,
		OnClickListener {
	...
	public void onLoadMore(int id) {
		...
		if (!MAX_Length) {
			...
			Toast.makeText(A2_SearchActivity.this, "只有这么多商品哟",
			...
			if (result == 1 && statusCode == 200) {
				...
				try {
					...
					if (array.length() - D_SIZE < 20) {
						Log.i("datasize", D_SIZE + "true");
						MAX_Length = true;
					} else {
						Log.i("datasize", D_SIZE + "false");
						MAX_Length = false;
					}
